 ...
 *  中奖订单详情
 */
class WinningInfo extends AbsOrder
{
    public $permission = [
        'execute' => 1,
        'edit' => 2
    ...
    {
        return $this->user->permission['212']??0;
    }
    ...
    public function execute(Request $request)
    {
        ...
        $data = DB::table('periods as p')
            ...
            ->where([['p.id', $id],['p.user_type','=', 0],['p.status',3]])
    ...
    public function edit(Request $request){
        ...
        if($remark) {
            $data['remark'] = $remark;
            $re = DB::table('periods')->where([['status',3],['user_type','=', 0],['id', $id]])->update($data);
        }

        if($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：{$id}，中奖订单编辑", 'type' => 3,'ip'=>$request->getClientIp()]);
