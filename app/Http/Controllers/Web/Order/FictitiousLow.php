...
class FictitiousLow extends AbsOrder
{
    ...
    {
        return $this->user->permission['218']??0;
    }
    ...
    public function execute(Request $request)
    {
        ...
        $query->where([['p.order_type', 1], ['p.status', 3], ['p.user_type', '=', 0], ['p.order_status', '>', 1],['p.order_status', '!=', 6],['goods_type',1]]);
    ...
    public function excel(Request $request)
    {
        ...
        try {
            ...
            unset($list);
            unset($query);
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "虚拟低价订单导出", 'type' => 5, 'ip' => $request->getClientIp()]);
    ...
    public function complete(Request $request)
    {
        ...
        $re = DB::table('periods')->where([['order_type', 1], ['status', 3], ['user_type', '=', 0],['goods_type',1]])->whereIn('amount', config('dui_ba.amount'))->whereIn('id', $id)->update($data);

        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：" . json_encode($id) . "，虚拟低价订单，夺宝完成", 'type' => 3, 'ip' => $request->getClientIp()]);
    ...
    public function orderStatus(Request $request)
    {
        ...
        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：" . json_encode($id) . "，虚拟低价订单，修改订单状态" . $data['order_status'], 'type' => 3, 'ip' => $request->getClientIp()]);
