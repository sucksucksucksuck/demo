 ...
 *  用户所有订单
 */
class UserOrder extends AbsOrder
{
    public $permission = [
        'execute' => 1,
        'excel' => 2
    ...
    {
        return $this->user->permission['217']??0;
    }
    ...
    public function execute(Request $request)
    {
        ...
        $total = $query->count();
        $list = $query
            ->select('o.id', 'o.periods_id', 'o.goods_id as o_goods_id', 'o.num', 'o.create_at', 'o.winning',
                'g.title',
                'p.lottery_show_at', 'p.periods', 'p.total', 'p.amount', 'p.contact_name', 'p.contact_id', 'p.contact_phone', 'p.status', 'p.order_status', 'p.order_type'
            )
            ->orderBy('o.id', 'desc')
            ->forPage($this->page, $this->page_size)
            ->get();

        $user_info = DB::table('user')->select('id', 'nickname', 'recharge_amount', 'residual_amount', 'winning_amount', 'exchange_amount')->where('id', $user_id)->first();
        if ($user_info) {
    ...
    public function excel(Request $request)
    {
        ...
        try {
            ...
            foreach ($list as $k => $v) {
                ...
                } else if ($v->winning == 1 && $v->order_type == 2) {
                    $arr['a'] = '-';
                    $arr['b'] = '-';
            ...
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "用户所有订单导出", 'type' => 5, 'ip' => $request->getClientIp()]);
