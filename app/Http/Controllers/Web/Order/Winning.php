...
class Winning extends AbsOrder
{
    ...
    {
        return $this->user->permission['201']??0;
    }
    ...
    public function execute(Request $request)
    {
        ...
        if (!empty($this->user->group_id)) {
            $group = DB::table('admin_group')->find($this->user->group_id, ['extend']);
            if (!empty($group->extend)) {
                $extend = json_decode($group->extend, true);
                $query->where([['p.status', 3], ['p.user_type', '=', $extend['user_type']]]);
            } else {
                $query->where([['p.status', 3], ['p.user_type', '=', 0]]);
            }
        } else {
            $query->where([['p.status', 3], ['p.user_type', '=', 0]]);
        }
        ...
        $list = $query
            ->select('p.id', 'p.order_id', 'p.goods_id', 'p.user_id', 'p.lottery_show_at', 'p.unit_price', 'p.lucky_code', 'p.buy', 'p.status', 'p.periods', 'p.order_status', 'p.amount', 'p.contact_name', 'p.contact_phone', 'p.contact_id', 'p.goods_type', 'p.user_type', 'p.payment_type', 'p.deliver_at', 'p.real_amount', 'p.order_type','p.confirm_at',
    ...
    public function excel(Request $request)
    {
        ...
        try {
            ...
            $list = $query
                ->select('p.id', 'p.order_id', 'p.goods_id', 'p.user_id', 'p.lottery_show_at', 'p.unit_price', 'p.lucky_code', 'p.buy', 'p.status', 'p.periods', 'p.order_status', 'p.real_amount', 'p.contact_name', 'p.contact_phone', 'p.contact_id', 'p.order_id', 'p.payment_channel', 'p.payment_no', 'p.deliver_at', 'p.order_status', 'p.order_type', 'p.payment_type','p.confirm_at',
            ...
            $payment_type2 = ['手动发货', '-', '拒绝发货', '成功发货'];
            $order_status_info = ['获得奖品-待确认', '订单确认', '盘古发货', '已签收', '夺宝完成', '打款失败'];
            $data = array(array('订单编号', '用户ID', '期数ID', '商品ID', '商品名称', '期数', '购买时间', '开奖时间','确认时间', '发货时间', '购买人次', '实付金额', '商品金额', '实际发货金额', '订单状态', '支付宝姓名', '支付宝账号', '收货人姓名', '联系方式', '收货地址', '操作状态', '操作人'));
            foreach ($list as $k => $v) {
            ...
            unset($list);
            unset($query);
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "中奖订单导出", 'type' => 5, 'ip' => $request->getClientIp()]);
    ...
    public function complete(Request $request)
    {
        ...
        $re = DB::table('periods')->where([['status', 3], ['user_type', '=', 0]])->whereIn('id', $id)->update($data);

        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：" . json_encode($id) . "，中奖订单，夺宝完成", 'type' => 3, 'ip' => $request->getClientIp()]);
