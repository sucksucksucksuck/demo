...
class Entity extends AbsOrder
{
    ...
    {
        return $this->user->permission['202']??0;
    }
    ...
    public function execute(Request $request)
    {
        ...
        $query->where([['p.order_type', 2], ['p.status', 3], ['p.order_status', '>', 1], ['p.user_type', '=', 0]]);
        ...
        $list = $query
            ->select('p.id', 'p.order_id', 'p.goods_id', 'p.user_id', 'p.lottery_show_at', 'p.unit_price', 'p.lucky_code', 'p.buy', 'p.status', 'p.periods', 'p.order_status', 'p.amount', 'p.contact_name', 'p.contact_phone', 'p.contact_id', 'p.payment_type', 'p.goods_type', 'p.confirm_at', 'p.deliver_at', 'p.real_amount',
                'g.title', 'g.icon', 'g.total', 'g.url',
                'u.nickname'//,
    ...
    public function excel(Request $request)
    {
        ...
        try {
            ...
            $list = $query
                ->select('p.id', 'p.order_id', 'p.goods_id', 'p.user_id', 'p.lottery_show_at', 'p.unit_price', 'p.lucky_code', 'p.buy', 'p.status', 'p.periods', 'p.order_status', 'p.real_amount', 'p.contact_name', 'p.contact_phone', 'p.order_id', 'p.payment_channel', 'p.payment_no', 'p.contact_id', 'p.deliver_at', 'p.payment_type','p.confirm_at',
            ...
            $data = array(array('订单编号', '用户ID', '期数ID', '商品ID', '商品名称', '期数', '购买时间', '开奖时间','确认时间', '发货时间', '购买人次', '实付金额', '商品金额', '实际发货金额', '收货人姓名', '联系方式', '收货地址', '快递公司', '快递单号', '订单状态', '发货状态', '操作人'));
            foreach ($list as $k => $v) {
                ...
                $arr['contact_phone'] = empty($v->contact_phone) ? '-' : $v->contact_phone;
    ...
    public function complete(Request $request)
    {
        ...
        $re = DB::table('periods')->where([['order_type', 2], ['status', 3], ['user_type', '=', 0]])->whereIn('id', $id)->update($data);

        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：" . json_encode($id) . "，实物订单，夺宝完成", 'type' => 3, 'ip' => $request->getClientIp()]);
            return ['errcode' => 0, 'msg' => '操作成功', 'data' => $re];
        } else {
            return ['errcode' => 1002, 'msg' => '没有修改', 'data' => ''];
        }
    }
    ...
    public function orderStatus(Request $request)
    {
        ...
        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：" . json_encode($id) . "，实物订单，修改订单状态" . $data['order_status'], 'type' => 3, 'ip' => $request->getClientIp()]);
            return ['errcode' => 0, 'msg' => '操作成功', 'data' => $re];
        } else {
