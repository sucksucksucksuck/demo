 ...
 *  虚拟订单（兑吧）
 */
class FictitiousExchange extends AbsOrder
{
    public $permission = [
        'execute' => 1,
        'excel' => 2,
    ...
    {
        return $this->user->permission['204']??0;
    }
    ...
    public function execute(Request $request)
    {
        ...
        $dui_ba_id = $request->input('dui_ba_id');
        ...
        if ($dui_ba_id) {
            $dui_ba = DB::table('dui_ba')->where('dui_ba_order_no', 'like', "%{$dui_ba_id}%")->pluck('periods_id');
            $query->whereIn('p.id', $dui_ba);
        }
        if ($start_time)
        ...
        $query->where([['p.order_type', 1], ['p.status', 3], ['p.user_type', '=', 0], ['p.order_status', '>', 1]]);
        $query->whereIn('p.amount', config('dui_ba.amount'));
    ...
    public function excel(Request $request)
    {
        ...
        try {
            ...
            $list = $query
                ...
                ->limit(10000)
                ->get();
            $payment_type = ['手动打款', '兑吧打款', '拒绝打款', '成功打款'];
            ...
            unset($list);
            unset($query);
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "虚拟订单（兑吧）导出", 'type' => 5, 'ip' => $request->getClientIp()]);
    ...
    public function complete(Request $request)
    {
        ...
        $re = DB::table('periods')->where([['order_type', 1], ['status', 3], ['user_type', '=', 0],])->whereIn('amount', config('dui_ba.amount'))->whereIn('id', $id)->update($data);

        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：" . json_encode($id) . "，虚拟订单(兑吧)，夺宝完成", 'type' => 3, 'ip' => $request->getClientIp()]);
    ...
    public function orderStatus(Request $request)
    {
        ...
        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：" . json_encode($id) . "，虚拟订单（兑吧），修改订单状态" . $data['order_status'], 'type' => 3, 'ip' => $request->getClientIp()]);
