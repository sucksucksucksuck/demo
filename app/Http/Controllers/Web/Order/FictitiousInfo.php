 ...
 *  虚拟订单详情
 */
class FictitiousInfo extends AbsOrder
{
    public $permission = [
        'execute' => 1,
        'edit' => 2,
    ...
    {
        return $this->user->permission['207']??0;
    }
    ...
    public function edit(Request $request)
    {
        ...
        $re = DB::table('periods')->where([['id', $id], ['user_type', '=', 0], ['order_type', 1], ['status', 3]])
            ->where(function ($query) {
    ...
    public function delivery(Request $request)
    {
        ...
        $where = [['id', $id], ['user_type', '=', 0], ['order_type', 1], ['status', 3]];
        //发货
        if ($order_status == 3) {
            if ($payment_type == 4) {
            ...
            if ($re['errcode']) {
                DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：{$id}，虚拟订单发货，" . $log[$log_index] . '（异常）', 'type' => 3, 'ip' => $request->getClientIp()]);
        ...
        $re = DB::table('periods')->where($where)
            ->where(function ($query) {
                $query->whereNotIn('amount', config('dui_ba.amount'));
                $query->orWhere(function ($query) {
                    $query->whereIn('amount', config('dui_ba.amount'))->where('goods_type', 0);
        ...
        if ($re) {
            if (in_array($order_status, [2, 3,6])) {
                DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：{$id}，虚拟订单发货，" . $log[$log_index], 'type' => 3, 'ip' => $request->getClientIp()]);
            }
    ...
    public function conversion(Request $request)
    {
        ...
        $where = [['id', $id], ['user_type', '=', 0], ['order_type', 1], ['status', 3], ['order_status', '<', 3]];
        $data['order_type'] = 2;
        $data['contact_name'] = $contact_name;
        $data['contact_id'] = $contact_id;
        ...
        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id：{$id}，虚拟订单转换实物", 'type' => 3, 'ip' => $request->getClientIp()]);
