...
class PrizeGoods extends AbsEventManage
{
    public $permission = [
        'execute' => 1,
        'goodsInfo' => 1,
    ...
    {
        return $this->user->permission['706']??0;
    }
    ...
    public function create(Request $request)
    {
        ...
        if (!$event_id || !$title || (!$chance && !is_numeric($count)) || !$group) {
        ...
        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id:{$re}，添加活动奖品", 'type' => 2, 'ip' => $request->getClientIp()]);
    ...
    public function edit(Request $request)
    {
        ...
        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id:{$id}，编辑活动奖品", 'type' => 3, 'ip' => $request->getClientIp()]);
    ...
    public function del(Request $request)
    {
        ...
        if ($re) {
            DB::table('admin_log')->insert(['admin_id' => $this->user->id, 'log' => "id:{$id}，删除活动奖品", 'type' => 3, 'ip' => $request->getClientIp()]);
