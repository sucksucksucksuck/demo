...
import * as eventAction from '../../../../action/event/prize_goods';
...
const module = 701;
class PrizeGoods extends AbsTabContent {
    constructor(props) {
        super(props, module);
        this.state = props['prizeGoods'];
        this.state.search.id = this.props.extend;

    }

    componentWillMount() {
        eventAction.getPrizeGoods(this.state.search);
    }
    ...
    onItemButtonClick(search, item, name) {
        if (name == "edit") {
            eventAction.goodsPopUp(item.id, search);
        }
        if (name == "del") {
...
export default connect(({prizeGoods}) => ({prizeGoods}))(PrizeGoods);
