...
class OrderEntityInfo extends React.Component {
    constructor(props) {
        super(props);
        //通过属性传递state过来
    ...
    componentWillMount() {
        orderAction.getEntityInfo({id: this.props.extend});
    }
    ...
    onSubmit() {
        utilAction.confirm("是否确认修改?", () => {
            orderAction.setEntityInfo(this.refs['form'].getValue(this.state.info));
            // orderAction.getEntityInfo({id: this.props.extend});
            return true;
        });
    }
...
export default connect(({orderEntityInfo}) => ({orderEntityInfo}))(OrderEntityInfo);
