...
class Fictitious extends AbsOrder {
    constructor(props) {
        super(props, module);
        this.state = props['orderFictitious'];
    }
    ...
    getSearch() {
        return [
            ...
            <div key="12" className="item">
                <div>虚拟商品付款</div>
                <Select name="payment_type" object={true} opts={fictitious} defaultText="全部"/>
            </div>,
    ...
    getColumnStyle(val, head, item) {
        ...
        if (head.type === "payment_type") {
            if (val == "5") {
                return <div className="warn">{fictitious[val]}</div>
            } else {
    ...
    onItemButtonClick(search, item, name) {
        if (name == "delivery") {
            orderAction.deliveryPopUp("fictitious", item, search);

        }
        if (name === "order-status") {
            orderAction.changeOrderStatus("fictitious", item['id'], item['order_status'], search);
        }
        if (head.type === "amount") {
            return <div>
                <div>{item.amount}</div>
    ...
    onSubmit() {
        ...
        orderAction.getFictitious(this.state.search);
    }

    componentWillUnmount() {
        // orderAction.fictitiousClear();
    }
    ...
    exportChecked() {
        ...
        if (ids.length) {
            orderAction.fictitiousExcel({id: ids});
        } else {
    ...
    getTableButton() {
        let btn = [
            <NavLink key="1" module="207" action="0" name="details" to="/order/fictitious/info/${id}">订单详情</NavLink>,
            <button key="2" module="207" action="2" compare="${order_status}==2" name="delivery">我要发货</button>,
            <button key="2" module="207" action="2" compare="${order_status}==3" name="delivery">修改发货信息</button>,
    ...
    complete() {
        ...
        if (ids.length !== 0) {
            utilAction.confirm("是否改变订单状态为'夺宝完成',请确认", () => {
                orderAction.ficitiousComplete({id: ids}, this.state.search);
                // orderAction.getFictitious(this.state.search);
                return true;
            });
        } else {
    ...
    onSale(type) {
        this.state.search.goods_type = type;
    ...
    getStatus() {
        return <div className="status">
                        <span className={this.state.search.goods_type === 1 ? "active" : null}
                              onClick={this.onSale.bind(this, 1)}>虚拟商品</span>
            <span className={this.state.search.goods_type === 0 ? "active" : null}
                  onClick={this.onSale.bind(this, 0)}>实物商品</span>
        </div>;
    }
...
export default connect(({orderFictitious}) => ({orderFictitious}))(Fictitious);
