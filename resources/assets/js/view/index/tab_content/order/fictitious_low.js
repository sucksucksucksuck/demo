...
class FictitiousLow extends AbsOrder {
    constructor(props) {
        super(props, module);
        this.state = props['orderFictitiousLow'];
    }
    ...
    //
    // getTableButton() {
    //     return <button name="details">详情</button>;
    // }

    onItemButtonClick(search, item, name) {
        if (name == "delivery") {
            orderAction.deliveryPopUp("fictitiousLow", item, search);

        }
        if (name === "order-status") {
            orderAction.changeOrderStatus("fictitiousLow", item['id'], item['order_status'], search);
        }
        if (head.type === "amount") {
            return <div>
                <div>{item.amount}</div>
    ...
    exportChecked() {
        ...
        if (ids.length) {
            orderAction.fictitiousLowExcel({id: ids});
        } else {
    ...
    getTableButton() {
        let btn = [
            <NavLink key="1" module="218" action="0" name="details"
                     to="/order/fictitious/low/info/${id}">订单详情</NavLink>,
            <button key="2" module="219" action="2" compare="${order_status}==2" name="delivery">我要发货</button>,
            <button key="2" module="219" action="2" compare="${order_status}==3" name="delivery">修改发货信息</button>,
    ...
    exportOption(e) {
        ...
        if ((this.state.end_time && this.state.start_time) || (this.state.deliver_start_time && this.state.deliver_end_time)) {
            orderAction.fictitiousLowExcel({
    ...
    complete() {
        ...
        if (ids.length !== 0) {
            utilAction.confirm("是否改变订单状态为'夺宝完成',请确认", () => {
                orderAction.ficitiousLowComplete({id: ids}, this.state.search);
                // orderAction.getFictitiousLow(this.state.search);
                return true;
            });
        } else {
...
export default connect(({orderFictitiousLow}) => ({orderFictitiousLow}))(FictitiousLow);
