...
class FictitiousDelivery extends React.Component {
    constructor(props) {
        super(props);
        //通过属性传递state过来
        this.state = props['orderFictitiousInfo'];
    ...
    componentDidMount() {  //render前
        orderAction.getFictitiousInfo({id: this.props.item.id, operation: 1});
    }
    ...
    componentWillUnmount() {
        ...
        orderAction.fictitiousUnlock({id: this.props.item.id});
        orderAction.clear();
    }
        ...
        if (this.state.info.payment_fee / 10000 > 10) {
            ...
            if (this.state.info.conversion == true) {
                if (this.state.info.order_status == 2) {
                    orderAction.fictitiousDelivery({
                ...
                orderAction.fictitiousConversion({
                    id: this.state.info.id,
                    contact_name: this.state.info.contact_name,
        ...
        return (
            <div className="dialog-form">
                    ...
                    <Form
                        ...
                        <table>
                            {this.state.info.conversion ?
                                ...
                                <tr>
                                    <td>流水号:</td>
                                    <td><Input allowNull={false} maxLength="50" verify="" name="payment_no"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td>备注:</td>
                ...
                <Button item={this.state.info}>
                    ...
                    <button compare="${order_status}==2&&${conversion}==true" module="207" action="3" name="3"
                            type="button" onClick={this.conversion.bind(this)}>转为实物
                    </button>
                    <button compare="${order_status}==2&&${conversion}==true" module="207" action="4" name="6"
                            type="button" onClick={() => {
                        orderAction.fictitiousDelivery({
                            id: this.state.info.id,
                            contact_name: this.state.info.contact_name,
                    ...
                    <button compare="${order_status}==2&&${conversion}==true" name="4" type="button" module="207" action="5"
                            onClick={() => {
                                orderAction.fictitiousDelivery({
                                    id: this.state.info.id,
                                    contact_name: this.state.info.contact_name,
                    ...
                    <button compare="${order_status}==2&&${conversion}==true" name="5" type="button"
                            onClick={this.refuse.bind(this)}>拒绝打款
                    </button>
                    <button name="7" type="button" onClick={this.onClick.bind(this, 'cancel')}>取消</button>
                </Button>
            </div>
        )
...
export default connect(({orderFictitiousInfo, moneyInfo}) => ({orderFictitiousInfo, moneyInfo}))(FictitiousDelivery);
