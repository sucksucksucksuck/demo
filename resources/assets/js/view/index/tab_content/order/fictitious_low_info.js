...
class OrderFictitiousLowInfo extends React.Component {
    constructor(props) {
        super(props);
        //通过属性传递state过来
    ...
    componentDidMount() {  //render前
        orderAction.getFictitiousLowInfo({id: this.props.extend});
    }
    ...
    onSubmit() {
        utilAction.confirm("是否确认修改?", () => {
            orderAction.setFictitiousLowInfo(this.refs['form'].getValue(this.state.info));
            return true;
        });
    }
...
export default connect(({orderFictitiousLowInfo}) => ({orderFictitiousLowInfo}))(OrderFictitiousLowInfo);
